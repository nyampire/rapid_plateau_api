...
| 14 | 取り込み器が 2 を返した (引数の不整合の可能性)。2 の予約を守るための写し |
...
# SHIP_PATH はサーバの PLATEAU_IMPORT_DIR と同じ絶対パスにする。
# ship_all.sh は都市の一覧を SHIP_PATH の親へ置くので、
# 親をサーバのホームにしておくと運用の手順が短くなる。
SHIP_HOST="user@example"
SHIP_PATH="$HOME/plateau_import"
